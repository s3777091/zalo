...
class ChatMessageHistory:
    ...
    async def add_messages(self, messages: List[BaseMessage]):
        ...
        await self._save_to_db(filtered)
        await self._save_to_cache(filtered)

    async def summarize_if_needed(self):
        current_cache_messages = await self._load_from_cache()
        if not current_cache_messages:
            return

        summary_message = current_cache_messages[0] if isinstance(current_cache_messages[0], SystemMessage) else None
        messages_since_summary = current_cache_messages[1:] if summary_message else current_cache_messages
        ...
        try:
            last_few_messages = current_cache_messages[-2:]
            messages_to_summarize = current_cache_messages[:-2]
    ...
    async def _load_from_cache(self) -> List[BaseMessage]:
        try:
            ...
            if raw_messages:
                ...
                messages = [_deserialize(m) for m in reversed(raw_messages)]
                filtered = []
                for m in messages:
                    if isinstance(m, ToolMessage):
                        continue
    ...
    async def _load_from_db(self) -> List[BaseMessage]:
        ...
        try:
            rows = await db_manager.fetch_all("SELECT is_bot, text FROM public.chat_histories WHERE from_id = $1 ORDER BY created_at ASC", [self.user_id])
            ...
            if filtered:
                await self._overwrite_cache(filtered)
            return filtered
        except Exception as e:
    ...
    async def _save_to_db(self, messages: List[BaseMessage]):
        try:
            ...
            for m in safe_messages:
                ...
                params = (is_bot, message_id, text, self.user_id)
                await db_manager.execute(query, params)

        except Exception as e:
            logger.error(
                f"Error saving to PostgreSQL for user '{self.user_id}': {e}", exc_info=True
            )
